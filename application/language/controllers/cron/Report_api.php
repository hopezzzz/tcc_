  ...
  public function get_report($request_id='NULL',$user_id='')
  {
    ...
    if(count($users) > 0)
    {
     ...
     if(is_file($res['report_file']))
     {
	   ...
	   if($usr['request_type'] == '_GET_MERCHANT_LISTINGS_DATA_' || $usr['request_type'] == '_GET_MERCHANT_LISTINGS_INACTIVE_DATA_' )
          ...
          {
              $this->report_process->process_afn_inventory_data($usr['profile_id'],$res['report_file'],$usr['country_code'],$usr['request_type']);
          }
       elseif($usr['request_type'] == '_SC_VAT_TAX_REPORT_')
       {
           $this->report_process->process_vat_tax_data($usr['profile_id'],$res['report_file'],$usr['country_code'],$usr['request_type']);
       }
	...
	elseif($usr['request_type']=='_GET_FLAT_FILE_ALL_ORDERS_DATA_BY_LAST_UPDATE_')
	 ...
	 elseif($usr['request_type']=='_GET_FLAT_FILE_ACTIONABLE_ORDER_DATA_')
		...
		elseif($usr['request_type']=='_GET_VAT_TRANSACTION_DATA_')
        {
            $this->report_process->process_vat_transaction_data($usr['profile_id'],$res['report_file'],$usr['country_code'],$usr['request_type']);
        }
        else
        {
