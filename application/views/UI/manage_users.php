...
<div id="myModal" class="modal fade" role="dialog">
  ...
  <div class="card panel ">
          ...
          <form  ng-submit="update_amazon_api()" name='amzForm' novalidate>
							  ...
							  <div class="pad"><b>Password</b></div>
 ...
 -->
    ...
    <div class="card panel ">
      <div class="panel-heading">
			 ...
			 <div class="pull-right" style="margin-right:30px;"><a href='#' class="btn btn-primary"  data-toggle="modal" ng-click='reset()' data-target="#myModal">ADD User</a></div>
      </div>
      ...
      <div class="panel-body table-responsive" >
        ...
        <table class='table'>
         <thead>
           <tr><th>SL.No</th><th>Name</th><th>Email</th><th>Password</th><th>Position</th><!---<th>Verify Status</th>---><th>Status</th><th>Action</th><!---<th>Amount Paid</th>---><th>Delete User</th></tr>
         </thead>
         <tbody>
          <tr ng-repeat='usr in user_list'>
			...
			<td>{{usr.password}}</td>
            <td>
			<span ng-if='usr.uid==1'  class="label label-info">Super Admin</span>
			<span ng-if='usr.scr_is_admin==1 && usr.uid!=1' ng_click='make_user_normal(usr)' class="label label-info">{{usr.position}}</span>
                <span ng-if='usr.scr_is_admin==2' ng_click='make_user_admin(usr)' class="label label-primary">{{usr.position}}</span></td>			
           ...
           <!--- <td>{{usr.total_amt}}</td>  --->
            <td ><a href='' ng-show='usr.is_active==0' class="label label-success" ng-click='trigger_active(usr)'>Activate User</a><a href=''  ng-show='usr.is_active==1' class="label label-danger" ng-click='trigger_delete(usr)'>Deactivate User</a> | <a href='' class="label label-info" ng-click='trigger_edit(usr)'>Edit User</a></td> 
        	<td><a href=''  class="label label-danger" ng-click='delete_user(usr)'>Delete User</a></td>
			</tr>
         </tbody>
        </table>
      </div>
...
var activate_user=function(user)
	...
	var delete_user=function(user)
    {
       return $http({
                      method: "post",
                      url:"<?php echo $baseurl.'manage_users/delete_user'?>",
                      data:{
                          user_id:user
                      }
                     }); 
	...
	var make_user_admin=function(user)
    {
       return $http({
                      method: "post",
                      url:"<?php echo $baseurl.'manage_users/make_user_admin'?>",
                      data:{
                          user_id:user
                      }
                     }); 
	...
	var make_user_normal=function(user)
    {
       return $http({
                      method: "post",
                      url:"<?php echo $baseurl.'manage_users/make_user_normal'?>",
                      data:{
                          user_id:user
                      }
                     }); 
