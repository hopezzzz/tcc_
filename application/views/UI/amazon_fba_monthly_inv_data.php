...
<div id="myModal" class="modal fade" role="dialog" style="margin-left:150px;">
  ...
  <div class="card panel ">
    ...
    <div class="row">
        ...
        <div class="card panel ">
            <div class="panel-heading">
                   <h3 class="panel-title"><i class="fa fa-database"></i> FBA Monthly Inventory History Report</h3>
            </div>
            <div class="panel-body" >
            <div class="row">
            <div class="col-sm-12">
                                  ...
                                  <table class="table table-striped jambo_table bulk_action">
                                    ...
                                    <tbody>
                                        <tr    ng-show='prodList.length>0' ng-repeat="tnx in prodList " >
                                            <td >{{tnx.prod_month}}</td>
											<td >{{tnx.prod_fn_sku}}</td>
											<td >{{tnx.prod_sku}}</td>
											<td >{{tnx.prod_name}}</td>
										    <td >{{tnx.prod_avg_qty}}</td>
											<td>{{tnx.prod_qty}}</td>
											<td >{{tnx.prod_full_id}}</td>
											<td >{{tnx.prod_disp}}</td>
											<td >{{tnx.prod_country}}</td>
									   </tr>
...
crawlApp.factory('productFactory', ['$http', '$q','limitToFilter', function($http,$q,limitToFilter) {
    ...
    var product_list_url        =   "<?php echo $baseurl."amazon_fba_monthly_inv_data/get_product_list/"?>";
	var d_path        =   "<?php echo $baseurl."amazon_fba_monthly_inv_data/export_datasss/"?>";
	 var export_list_url        =   "<?php echo $baseurl."amazon_fba_monthly_inv_data/export_data"?>";
    ...
    var update_product_info=function(product_info)
    {
       return $http({
                      method: "post",
                      url:"<?php echo $baseurl.'amazon_fba_monthly_inv_data/update_product_info'?>",
                      data:{
