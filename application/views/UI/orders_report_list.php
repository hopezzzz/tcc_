...
<div id="myModal" class="modal fade" role="dialog" style="margin-left:150px;">
  ...
  <div class="card panel ">
    ...
    <div class="row">
        ...
        <div class="card panel ">
            <div class="panel-heading">
                   <h3 class="panel-title"><i class="fa fa-database"></i> Flat File Order Report</h3>
            </div>
            <div class="panel-body" >
            <div class="row">
            <div class="col-sm-12">
                                  ...
                                  <table class="table table-striped jambo_table bulk_action">
                                    ...
                                    <tbody>
                                        <tr  ng-click='show_order_details(tnx)' data-target="#myModal" data-toggle="modal"   ng-show='prodList.length>0' ng-repeat="tnx in prodList " >
                                            <td ng-click='show_order_details(tnx)'>{{tnx.ord_id}}</td>
                                            <td ng-click='show_order_details(tnx)'>{{tnx.ord_itm_id}}</td>
											<td>{{tnx.sku}}</td>
                                            <td>{{tnx.itm_name | limitTo:45}}</td>
                                            <td>{{tnx.price}}</td>
                                            <td>{{tnx.ship_price}}</td>
                                            <td>{{tnx.qty}}</td>
                                            <td>{{tnx.order_total}}</td>
										    <td>{{tnx.country}}</td>
...
crawlApp.factory('productFactory', ['$http', '$q','limitToFilter', function($http,$q,limitToFilter) {
    ...
    var product_list_url        =   "<?php echo $baseurl."orders_report_list/get_product_list/"?>";
	var d_path        =   "<?php echo $baseurl."orders_report_list/export_datasss/"?>";
	 var export_list_url        =   "<?php echo $baseurl."orders_report_list/export_data"?>";
    ...
    var update_product_info=function(product_info)
    {
       return $http({
                      method: "post",
                      url:"<?php echo $baseurl.'orders_report_list/update_product_info'?>",
                      data:{
