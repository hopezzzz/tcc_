...
class Amazon_fba_monthly_inv_data_model extends CI_Model
	{
	 ...
	 public function get_product_list($orderby='prod_id',$direction='DESC',$offet,$limit,$searchterm='')
    {
		...
		// print_r($cnt);
         $sqlcount="SELECT count(*) as total from rep_fba_monthly_inv_data WHERE user_id={$this->user_id}";
         $sqlquery= "SELECT  prod_month,prod_fn_sku,prod_sku,prod_name,prod_avg_qty,prod_qty,prod_full_id,prod_disp,prod_country FROM rep_fba_monthly_inv_data WHERE user_id={$this->user_id}";

        
        if(!empty($srterm) || $srterm !='')
        {
          $sqlquery.=" AND (prod_fn_sku LIKE '%".$srterm."%' OR prod_sku LIKE '%".$srterm."%') "; 
          $sqlcount.=" AND (prod_fn_sku LIKE '%".$srterm."%' OR prod_sku LIKE '%".$srterm."%') "; 
        }
