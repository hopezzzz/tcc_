...
class Orders_report_flat_list_model extends CI_Model
	{
	 ...
	 public function get_product_list($orderby='po_date',$direction='DESC',$offet,$limit,$searchterm='')
    {
		...
		// print_r($cnt);
         $sqlcount="SELECT count(*) as total from rep_flat_orders_list WHERE user_id={$this->user_id}";
         $sqlquery= "SELECT order_id,order_item_id,DATE_FORMAT(po_date,'%Y-%m-%d') AS po_date,DATE_FORMAT(pay_date,'%Y-%m-%d %H-%i-%s') AS pay_date,buy_name,buy_email,buy_phone,sku,REPLACE(title,'?',' ') AS title,qty,itm_price,itm_tax,ship_price,ship_tax,ship_addr1,ship_addr2,ship_addr3,ship_city,ship_post,ship_state,ship_country,itm_promo_discount,ship_promo_discount,country  FROM rep_flat_orders_list WHERE user_id={$this->user_id}";
	...
	public function export_data($searchterm='')
    {
         ...
         $sqlcount="SELECT count(*) as total from rep_flat_orders_list WHERE user_id={$this->user_id}";
         $sqlquery= "SELECT order_id,order_item_id,DATE_FORMAT(po_date,'%Y-%m-%d') AS po_date,DATE_FORMAT(pay_date,'%Y-%m-%d %H-%i-%s') AS pay_date,buy_name,buy_email,buy_phone,sku,REPLACE(title,'?',' ') AS title,qty,itm_price,itm_tax,ship_price,ship_tax,ship_addr1,ship_addr2,ship_addr3,ship_city,ship_post,ship_state,ship_country,itm_promo_discount,ship_promo_discount,country,ship_phone  FROM rep_flat_orders_list WHERE user_id={$this->user_id}";
