...
class Orders_report_last_update_list_model extends CI_Model
	{
	 ...
	 public function get_product_list($orderby='prod_id',$direction='DESC',$offet,$limit,$searchterm='')
    {
		...
		// print_r($cnt);
         $sqlcount="SELECT count(*) as total from rep_orders_update_list WHERE user_id={$this->user_id}";
         $sqlquery= "SELECT order_id,mer_order_id,DATE_FORMAT(po_date,'%Y-%m-%d') AS po_date,DATE_FORMAT(last_update_date,'%Y-%m-%d') AS last_update_date,ord_status,fulfillment AS fillfillment,REPLACE(itm_status,'Cancelled','Canceled') AS itm_status,ord_sku,asin,title,qty,sales_channel,itm_price,itm_tax,ship_price,ship_tax,gift_price,gift_tax,itm_promo_discount,itm_ship_discount,ship_city,ship_state,ship_country,ship_post,promo_id  FROM rep_orders_update_list  WHERE user_id={$this->user_id}";
	...
	public function export_data($searchterm='')
    {
         ...
         $sqlcount="SELECT count(*) as total from rep_orders_update_list WHERE user_id={$this->user_id}";
         $sqlquery= "SELECT order_id,mer_order_id,DATE_FORMAT(po_date,'%Y-%m-%d') AS po_date,DATE_FORMAT(last_update_date,'%Y-%m-%d') AS last_update_date,ord_status,fulfillment AS  fillfillment,itm_status,ord_sku,asin,title,qty,sales_channel,itm_price,itm_tax,ship_price,ship_tax,gift_price,gift_tax,itm_promo_discount,itm_ship_discount,ship_city,ship_state,ship_country,ship_post,promo_id  FROM rep_orders_update_list  WHERE user_id={$this->user_id}";
