...
class Orders_report_fba_ship_data_list_model extends CI_Model
	{
	 ...
	 public function get_product_list($orderby='f_id',$direction='DESC',$offet,$limit,$searchterm='')
    {
		...
		// print_r($cnt);
         $sqlcount="SELECT count(*) as total from rep_fba_fulfill_ship_data_list WHERE added_by={$this->user_id}";
         $sqlquery= "SELECT amz_order_id,sku,fnsku,asin,qty,itm_price,ship_price,gift_price,ship_city,ship_state,ship_post,DATE_FORMAT(ship_date,'%Y-%m-%d') as ship_date FROM rep_fba_fulfill_ship_data_list  WHERE added_by={$this->user_id}";

        
        if(!empty($srterm) || $srterm !='')
        {
          $sqlquery.=" AND (amz_order_id LIKE '%".$srterm."%' OR itm_price LIKE '%".$srterm."%' OR asin LIKE '%".$srterm."%' ) "; 
          $sqlcount.=" AND (amz_order_id LIKE '%".$srterm."%' OR itm_price LIKE '%".$srterm."%' OR asin LIKE '%".$srterm."%' ) "; 
        }
		   if(!empty($cnt) || $cnt !='')
        {
		...
		if(!empty($from_date))
        {
          ...
          $sqlquery.=" AND  ship_date >= ".$this->db->escape($from_date.' 00:00:00')." AND ship_date <=".$this->db->escape($to_date.' 23:59:00');
          $sqlcount.=" AND  ship_date >= ".$this->db->escape($from_date.' 00:00:00')." AND ship_date <=".$this->db->escape($to_date.' 23:59:00');
        }
