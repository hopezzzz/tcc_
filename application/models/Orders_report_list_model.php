...
class Orders_report_list_model extends CI_Model
	{
	 ...
	 public function get_product_list($orderby='ord_id',$direction='DESC',$offet,$limit,$searchterm='')
    {
		...
		// print_r($cnt);
         $sqlcount="SELECT count(*) as total from rep_converger_orders_data_list WHERE user_id={$this->user_id}";
         $sqlquery= "SELECT ord_id,ord_itm_id,REPLACE(itm_name,'?',' ') AS itm_name,sku,po_date,pay_date,price,ship_price,qty,order_total,country,buyer_name,buyer_email,ship_addr1,ship_addr2,ship_city,ship_zip,ship_state,ship_country  FROM rep_converger_orders_data_list  WHERE user_id={$this->user_id}";

        
        if(!empty($srterm) || $srterm !='')
        {
          $sqlquery.=" AND (ord_id LIKE '%".$srterm."%' OR ord_itm_id LIKE '%".$srterm."%' OR sku LIKE '%".$srterm."%' OR itm_name LIKE '%".$srterm."%') "; 
          $sqlcount.=" AND (ord_id LIKE '%".$srterm."%' OR ord_itm_id LIKE '%".$srterm."%' OR sku LIKE '%".$srterm."%' OR itm_name LIKE '%".$srterm."%') "; 
        }
		   if(!empty($cnt) || $cnt !='')
        {
	...
	public function export_data($searchterm='')
    {
        ...
        if(!empty($srterm) || $srterm !='')
        {
          $sqlquery.=" AND (ord_id LIKE '%".$srterm."%' OR ord_itm_id LIKE '%".$srterm."%') "; 
