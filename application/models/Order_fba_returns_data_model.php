...
class Order_fba_returns_data_model extends CI_Model
	{
	 ...
	 public function get_product_list($orderby='return_date',$direction='DESC',$offet,$limit,$searchterm='')
    {
		...
		// print_r($cnt);
         $sqlcount="SELECT count(*) as total from amz_order_return_data WHERE ret_for={$this->user_id} ";
         $sqlquery= "SELECT return_date,order_id,sku,asin,fn_sku,prod_name,qty,fullfill_cent_id,detailed_disp,reason,status,licence_plate_num,cust_comments FROM amz_order_return_data  WHERE ret_for={$this->user_id} ";
