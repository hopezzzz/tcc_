...
class Subscription_fee_data extends CI_Controller {
  public function  __construct()
  {
     ...
     else
     {
       ...
       $this->load->model('Subscription_fee_data_model');
  ...
  public function index()
  {
      ...
      $this->load->view('UI/subscription_fee_data');
      $this->load->view('UI/footer');
  }
  
   public function get_product_list($orderby='posted_date',$direction='DESC',$offet,$limit,$searchterm='')
  {
      $result_set=$this->Subscription_fee_data_model->get_product_list($orderby,$direction,$offet,$limit,$searchterm);
	...
	public function export_data($searchterm='')
  {
      ...
      $product=$this->Subscription_fee_data_model->export_data($searchterm);
      if(!empty($product))
      {
		...
		if($status=='csv')
		{
        fputcsv($feed_file, array('Settlement ID','Transaction Type','Date','Amount'));
        foreach($product as $prd)
        {
          $data=array($prd['settlement_id'],$prd['transaction_type'],$prd['posted_date'],$prd['other_amt']);
