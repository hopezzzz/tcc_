...
class My_profile extends CI_Controller {
  ...
  public function update_amazon_api()
  {
      if(isset($_POST['api_detail']))
      {
         ...
         if(isset($api->seller_id)   && isset($api->tokenid) && isset($api->access_key) && isset($api->secret_key)  && !empty($api->seller_id) && !empty($api->tokenid) && !empty($api->access_key) && !empty($api->secret_key))
         {
           if($this->update_api($api->seller_id,$api->tokenid,$api->access_key,$api->secret_key))
            {
  ...
  public function update_api($seller_id,$token_id,$access_key,$secret_key)
    {
       $time_stamp=date("Y-m-d H:s:i");
       $sql="INSERT INTO amazon_profile(profile_id,seller_id,auth_token,access_key,secret_key) VALUES('{$this->user_id}','{$seller_id}','{$token_id}','{$access_key}','{$secret_key}')
  ...
  public function get_profile_info()
  {
     $sql="SELECT scr_u_id as uid,scr_firstname as fname,scr_lastname as lname,scr_uname as email,scr_is_verified as is_verified,scr_active as is_active,trial_count as credits,
                 DATE_FORMAT(joined_on,'%Y-%m-%d') as joined,profile_img as pro_img         FROM `scr_user` WHERE scr_u_id={$this->user_id}";
